...
async def createActiveTowerRun(
    ...
    if not user.admin:
        raise HTTPException(
            status_code=403,
            detail="Você não tem permissão para criar níveis da torre!")
...
async def desativateActiveTowerRun(
    ...
    if not user.admin:
        raise HTTPException(
            status_code=403,
            detail="Você não tem permissão para desativar níveis da torre!")
    ...
    if not active_tower_run:
        raise HTTPException(
            status_code=404,
            detail="Nível da torre não encontrado!")
...
async def activateActiveTowerRun(
    ...
    if not user.admin:
        raise HTTPException(
            status_code=403,
            detail="Você não tem permissão para ativar esse nível da torre!")
