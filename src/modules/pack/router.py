...
async def ListHeroPack(
    ...
    result = await listHeroPack(
        session=session, 
        id_hero_pack=id_hero_pack)
    return result

@pack_routes.get("list-All-hero-pack")
async def ListAllHero(
    session: Session = Depends(get_session)
    ):
    result = await listAllHeroPack(
        session=session)
    return result